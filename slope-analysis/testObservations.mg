...
for counter := 1 to #L1 do
	...
	if 2*k/3 in Old1 then
		printf "Newform eigenvalue detected in oldforms for k=%o, i=1.\nMultiplicities in T1:%o, U1P0:%o, U1P2: %o.\n", k, Multiplicity(T1, 2*k/3), Multiplicity(U1P0, 2*k/3), Multiplicity(U1P2, 2*k/3);
	end if;
	if k/3 in Old2 then
		printf "Newform eigenvalue detected in oldforms for k=%o, i=2.\nMultiplicities in T2:%o, U2P0:%o, U2P2: %o.\n", k, Multiplicity(T2, k/3), Multiplicity(U2P0, k/3), Multiplicity(U2P2, k/3);
	end if;

	assert IsEmpty(U1 diff Old1) or MultisetToSet(U1 diff Old1) eq {2*k/3};
	assert IsEmpty(U2 diff Old2) or MultisetToSet(U2 diff Old2) eq {k/3};
